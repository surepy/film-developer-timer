...
<script lang="ts">
	...
	$: duration_seconds = Math.trunc(duration % 60)
		.toString()
		.padStart(2, '0');
	...
	$: remaining_minutes = Math.trunc(remaining_time / 60)
		.toString()
		.padStart(2, '0');
	// second component of remaining time
	$: remaining_seconds = Math.trunc(remaining_time % 60)
		.toString()
		.padStart(2, '0');
	...
	$: sub_remaining_seconds = Math.trunc(sub_remaining_time % 60)
		.toString()
		.padStart(2, '0');
	...
	async function startTimer() {
		console.log(
			`starting timer for start_time=${start_time}, end_time=${end_time}, duration=${duration}`
		);
...
<div class="flex flex-col place-content-center items-center justify-center">
	<Progressbar
		animate
		easing={sineOut}
		size="h-4"
		class="mt-2 w-44"
        color="red"
		progress={!$started ? 0 : (100 * (duration - remaining_time)) / duration}
	/>
	...
	{#if sub_timer}
		<Progressbar
			...
			progress={!$started
				? 0
				: (100 * (sub_timer_duration - sub_remaining_time)) / sub_timer_duration}
		/>
	...
	<div class="mt-3">
		{#if !$started}
			{#if start_time && remaining_time <= 0}
				{#if next_url}
					<Button color="purple" href={`${base}${next_url}`}>next</Button>
				{:else}
					<Button
						color="yellow"
						on:click={() => {
        ...
        -->
			<Button color="red" on:click={cancelTimer}>Cancel</Button>
		{/if}
	</div>
</div>
